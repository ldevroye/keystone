...
\begin{itemize}
    ...
    \item \verb|EAPP_BREAK_EVEN_CSV_OUTPUT|: output a csv format in order to produce graphs (\ref{fig:threshold_k})
\end{itemize}
...
This threshold is obtained when the ratio shown below is greater or equal to one.
\begin{equation}
...
\begin{align*}
C_{compute} &\geq \frac{40.000.000}{K} + \frac{40.000.000}{N}\\
C_{compute} &\geq 40.000.400
\end{align*}
...
\begin{figure}[H]
        ...
        \caption{Threshold for 1 error over a 100.000 runs enclave}
        \label{fig:threshold_k}
    \end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{img/plots/break_even_thresholds_runs_100000.png}
    \caption{Threshold ratio for k=$[0-9]$, over a 100.000 runs enclave}
    \label{fig:threshold_all_k}
\end{figure}
