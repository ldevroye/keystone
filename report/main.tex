...
In our implementation, we will see better results until $X\leq 12 248$, after that, the recovery cost starts balancing out the gains made by avoiding the save computation.
\begin{figure}[H]
    \centering
...
\begin{figure}[H]
    ...
    \caption{Optimal checkpoint interval for $10k$ runs and \textbf{1 error}, over for k=$[0-9]$, $15m$ load/save cost and $200k$ compute cost}
...
\begin{figure}[H]
    ...
    \caption{Optimal checkpoint interval for $100k$ runs and \textbf{1 error}, over for k=$[0-9]$, $15m$ load/save cost and $200k$ compute cost}
...
\begin{figure}[H]
    ...
    \caption{Optimal checkpoint interval for $1m$ runs and \textbf{1 fault}, over for k=$[0-9]$, $15m$ load/save cost and $200k$ compute cost}
...
%%%%%%%%%%%%%%%%%%%%%%%% Conclusion %%%%%%%%%%%%%%%%%%%%%%%%
\chapter{Conclusion}
This thesis asked whether rewind on RISC-V enclaves in an untrusted environment is practical and can be linked to the base papers \cite{sdrad,ARM-TrustZone-CortexM}.
The answer is that it can bef  practical, but only under a narrow set of conditions where the checkpoint cost is justified by enough useful execution between faults.

To support that answer, we combined a background study on SDRaD, secure intermittent computing, Keystone, and TrustZone-style checkpointing with an implementation of checkpoint and recovery inside a Keystone-based enclave.
The resulting design shows that the enclave can preserve state outside the normal execution flow while keeping the recovery mechanism small enough to remain meaningful as a security boundary.

The experimental analysis then showed that the cost model is dominated by checkpointing overhead and by the tradeoff between saving more often and recomputing more after a fault.
The break-even threshold and the checkpoint interval therefore matter more than a single fixed execution cost, because they determine whether rewind actually saves work in practice.
Furthermore, even under sub-optimal settings, rewinding could find some use for the time cost. In a non automated system that needs constant supervision for example.  

In addition, we saw that the security assumptions are not optional details.
The host may be untrusted, but the sealing key, the enclave boundary, and the restore path must remain trustworthy for the mechanism to remain sound.
This makes rewind a useful recovery technique for long-running protected workloads, but not a universal solution for every enclave or fault pattern.

The main contribution of this work is therefore not that rewind always wins, but that it can be analyzed, implemented, and justified in a concrete Keystone setting.
That result also points directly to the next steps of the thesis, especially better policies, lighter protection schemes, and richer fault models for analysis.
