...
\begin{itemize}
    ...
    \textit{host-facing, opaque} container holding the encrypted payload,
...
\begin{itemize}
    \item \textbf{CBC-MAC}: a Message Authentication Code built by
    ...
    \item \textbf{CTR (Counter mode)}: a block-cipher mode that turns
    ...
    \item \textbf{IV / Nonce / Seq}: the Initialization Vector
    ...
    a fixed-size value that the verifier recomputes and compares
...
For example, if the \verb|HOST_TESTING| is set to false, none of the host-side tests will be launched.
Also note that if the \verb|EAPP_TESTING| and the \verb|HOST_TESTING| are true, the host results could be useless or could even tamper with the enclave tests.
