...
register_call(OCALL_PRINT_BUFFER, 
            print_buffer_dispatch);
register_call(OCALL_SAVE_CHECKPOINT_BLOB, 
            save_checkpoint_blob_dispatch);
register_call(OCALL_LOAD_CHECKPOINT_BLOB, 
            load_checkpoint_blob_dispatch);

// 2. Basic edge call structure
edge_call_wrapper(void *buffer) 
{
...
EAPP_RETURN(0);
\end{verbatim}
In contrast to the host, the edge calls, being introduced by the enclave are sequential to the main loop:
\begin{verbatim}
// 1. The edge functions are
...
edge_call_wrapper(void *buffer) 
{
    auto call_status = CALL_STATUS_OK;
    // 3. Checks the call sanity  
    if (edge_call_args_ptr(edge_call, &arg_ptr, &arg_size) != 0 
        || arg_size == 0) 
    {
