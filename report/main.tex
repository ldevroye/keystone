...
\TODO{Review + small text}

\subsection{Assumptions}
The attacker may obtain arbitrary access to process memory and may trigger run-time attacks (faults) that corrupt application state, but cannot modify code pages under a W\(\oplus\)X policy \cite{sdrad, security-privacy}. 
...
For the intermittent-device context, the checkpointed state may be exposed to physical disruption such as power loss or tampering with external storage, so recovery data must be authenticated and protected against rollback or corruption.
...
An enclave state must remain protected across enclave transitions, that recovery metadata must be authenticated, and that the host must not be able to forge, tamper with, or roll back a checkpoint without detection. 
Also, the checkpointed state should also be encrypted; integrity and continuity remain the minimum security target. 
The design should also preserve the smallest possible trusted computing base and keep the rewind path testable and bounded as given by the Keystone architecture.
...
To address this security issue, the checkpoint is sealed (using the sealing key given by the SM) and uses a tag. This ensure that the host can neither read it or tamper with it.
Upon unsealing a the checkpoint, the enclave checks its tag so see if it fits the data. If not, the  data is discarded and the enclave restarts the computation from the beginning.
\\
