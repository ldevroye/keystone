...
The trade-off is clear: liveness is bought at the price of sealing, unsealing, and a Security Monitor willing to rebuild trust from nothing.
\end{abstract}
...
Making it mixed-criticalities enclaves of a bigger system.
...
To this extent, we will base our work on \textit{Keystone} that provides a platform to emulate a RISC-V processor with enclaves and cryptographic soundness using hardware (PMP) on a TEE. 
...
The basic workflow os to build the eapps, the host and then the \textit{possibly modified} runtime binaries.
...
The final reslut is another key pair, the SM key pair (\texttt{PK\_SM, SK\_SM}).
...
\end{figure}
Our implementation is based, on the Keystone basis.
This means that all of the enclave side (blue), is considered safe and all the outerworld (red) unsafe.
The enclave's never communicates with the host directly, the only shared space they have is the shared memory. 
This space is solely used to save and load the sealed checkpoint blob. 
The enclave then saves it, after the unsealing protocole, in its safe memory.
...
The discard and rewind is done by asking the SM to destroy the enclave, free the PMP entry and then allocate a new entry on the same binary to create the rewinded version of the enclave.
...
\begin{itemize}
    ...
    \item \verb|CHECKPOINT_INTERVAL|: sets the period the checkpointing, ie. the number of steps to do in between saves
...
&\approx 12.248\\
\end{align*}
Thus, until the interval gets to 12 thousand, the ratio will improve, this effect is more visible when the computation cost is closer to the save/load cost or when the numbers of runs gets smaller.  
...
This would be equal to a naïve enclave. 
\\
...
The balance between the two effect gives some kind of saturation slop.
...
However, the results rises until $X\leq 12 248$, after that, the recovery cost starts balancing out the gains made by avoiding the save computation.

As predicted, the cost of having a wider interval is growing along with the chances of faults occuring.
...
This does not add, according to us, any valuable results but rather, change the unit of measurment by doing a simple calculation.
...
We now have to consider the possible tampering and side-attacks, the former is that the attackers tries to mess with the blob bytes directly, which is not plausible under the scheme \textit{over a fixed-length message}. 
Another attack is to obtain information using side-channels information. This can be time to compute the tag or reloading the enclave by destroying it at every save edge call and XOR-ing the resulting initial ciphertexts.
...
Another point about the saving ratio is for mixed-criticality systems. The period could be mapped to a criticality level so that most important task get reloaded quicker while the others could still recover.
...
Note that this would need some fine tuning and system design in order to fit the hardware capabilites and task deadlines.
...
This ould also let the model capture the difference between isolated failures and adversarial or unstable environments. 
...
A strong fault model could enable to produce deeper researches and more relevant results to analyze. 
...
The work could be done over the whole enclave's or just the checkpointed structure using different methods to see and analyze the results. 
...
A trade-of between optimisation and cybersecurity. 
Making both of these could also help in differentiating a fault from an error and the possible recoverablity of the enclave.

\subsection{Continuous and asynchronous systems}
The last direction that we will discuss is to extend the enclave's work to continous systems. 
Such a system could have sensors that give information at not synchronized times, making the enclave work less naïve than iterating over a work to be done.
...
We were able to learn a lot about the trade-offs, the limitations and the overall designing skills that comes along managing both aspect requires. 
...
We would first invest more effort in a formalisation of the recovery boundary and comprehension of the platform workflow, clearly specifying which parts of enclave state must be captured, which invariants must hold at each checkpoint, and how these invariants are re-established after rewind.  
This would help guide both implementation and testing, and make it easier to reason about corner cases.  
Second, we would design and integrate benchmarks and adversarial test harnesses earlier in the process, to evaluate performance and failure behaviour under more realistic workloads and attack scenarios.  
...
Finally, we would examine the interaction with other Keystone features and multi-enclave scenarios sooner, to understand how compartmentalisation and cross-enclave dependencies affect recovery.  
