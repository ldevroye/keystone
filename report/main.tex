...
\begin{itemize}
    \item \verb|make-examples.sh| is rather simple and serves as the compilation.
    \item \verb|run-rewind.exp| is the most complex one as qemu \ref{sub:qemu} is an emulation, inputs (and waiting times) are expected from the user.
    A \verb|.exp| script is perfectly suited since they are the same each time.
    \item \verb|make-run.sh| is a simple concatenation of the two other ones.
\end{itemize}
