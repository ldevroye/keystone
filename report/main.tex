...
This is accomplished using shared memmory between the two. 
More precisely using Outbound CALLs (\texttt{OCALL}). However, this is done by using offsets in memmory region, using solely offsets (neglecting virtual address pointers)
...
Now the runtime part allocates an \texttt{edge\_call\_t} structure in shared memmory. 
...
We can make an easy parallel between enclaves and domains as they are executed in an isolated execution context with controlled memmory (\ref{sub:pmp}) from the outer, untrusted, world.
The SM (\ref{sub:sm}) is an enclave factory and provides them with, as previously mentionned, controlled memmory.
\\
...
The checkpointed data is sent and retrieved (to and from the host's memmory space) using the enclave's key (\ref{sub:data-sealing}) using two custom edge calls, one for sending and one for retrieving. 
...
\subsection{Trusted}
\begin{itemize}
  \item Enclave code, runtime and memmory access (Keystone TEE, RISC-V, PMP isolation)
...
This safety does not hold if the enclave itself introduces non-determinism (uninitializedmemory, internal RNG/timer reads, etc.) before the point of divergence from a prior checkpointseq.
...
Also, the checkpointed state should also be encrypted; integrity and continuity remain the minimum security target. 
...
while (ret != 0  && retry_counter < MAX_RUNS) 
{
    ...
    //SM destroy the encalve upon completion
    enclave.run(&ret); 
}
...
EAPP_RETURN(0);
\end{verbatim}
As shown above, the edge calls in the enclave are sequential in the main loop.
\begin{verbatim}
...
The host supervises the enclave and the latter is, once running, sealed from the former.
...
One could implement a direct call from the enclave to the SM to external safe memmory with, for example, table pages protected by outside PMP entry or another "data enclave" that has a direct communication channel with the "runtime" one.
...
This limitation being addressed, the saved checkpoint must be sealed and must be checked in order to not introduce unsafe data into the enclave's memmory.
\\

\subsubsection{Host-side}
\begin{verbatim}
    WRAPPER EDGE CALL LOAD

    WRAPPER EDGE CALL SAVE
...
Thus, a small fault model is implemented : Appendix \ref{file:fault}. it is based of of the the SplitMix \cite{splitmix,splitmix1} algorithm. 
...
if (fault_should_trigger(&f_m)) 
{
    // 4. Exit then encalve with abnormal code
    // Keystone::Error::EnclaveInterrupted = 16, the enum
    EAPP_RETURN(16);
}
\end{verbatim}
Note that we treat a fault as an error. 
That is because the goal is to checkpoint and not to catch, SEUs, bursts or even permanently damaged (worn out) hardware.
...
Note that the logs produced can be found in the \verb|build-$PLATFORM$BITS/build.log| file.
...
The Rewind encalve is evaluated on its overhead computation time and data cost.
...
\end{itemize}
Note that manipulating the macros can get confusing as they are independant. 
