...
\begin{itemize}
    ...
    \item \verb|EAPP_BREAK_EVEN_CSV_OUTPUT|: output a csv format in order to produce graphs (\ref{fig:treshold-k-1})
\end{itemize}
...
\section{Test cases}
This section defines the test cases used to evaluate the checkpoint and rewind
mechanism and explains their relevance.
There are two parts: the \textit{Analysis} (\ref{sub:test-analysis}),
where we quantify execution costs and overhead, and the \textit{Cyber security}
(\ref{sub:test-cyber}), where we validate the threat model assumptions
and security objectives from \ref{sub:threat-model}.

\subsection{Cyber security}
\label{sub:test-cyber}
Each test focuses on:
\begin{itemize}
    \item the attacker goal
    \item the security objective under evaluation (confidentiality,
          integrity, atomicity, unclonability)
    \item the behavior of the enclave (e.g., detection and rejection of
          tampered checkpoints)
\end{itemize}

\subsection{Analysis}
\label{sub:test-analysis}
Each test specifies:
\begin{itemize}
    \item the enclave workload and macros used setup
    \item the fault occurence rate
    \item the metrics recorded (latency, CPU cycles, memory overhead, final success rate)
\end{itemize}
...
In this chatper, we will discuss the the computation cost of checkpointing each iterations and the security assumptions and their limitations.
...
\begin{itemize}
    ...
    \item $C_{event}$ the total number of cycles used by the $event$ (save, load, compute, etc.) 
\end{itemize}
...
\begin{align*}
C_{no\_checkpoint} = (C_{compute}) \cdot (N + \sum_{i=0}^{K-1}(errors[i]))
\end{align*}
Note that the sum of the error indexes ($\sum_{i=0}^{K-1}(errors[i])$) as an expected value of $K \cdot \lfloor \frac{N}{2} \rfloor$.
This is because the probability of a fault occuring at index $i$ over $[0, N]$ ($X_i$, which is equal to $errors[i]$) is $\frac{1}{N}$ as $X$ is iid. Thus:
\begin{align*}
\mathbb{E}[X] = \frac{N}{2} 
\end{align*}
The average (expected value) can be derived using basic probability:
\begin{align*}
\mathbb{E}\left[\sum_{i=0}^{K-1} \text{errors}[i]\right]
&= \mathbb{E}\left[\sum_{i=0}^{K-1} X_i\right] \\
&= \sum_{i=0}^{K-1} \mathbb{E}[X_i] \\
&= \sum_{i=0}^{K-1} \mathbb{E}[X] \quad \text{(since } X_i \text{ are i.i.d.)} \\
&= K \cdot \mathbb{E}[X] \\
&= K \cdot \frac{N}{2}
\end{align*}
One can test the validity of this statement in the implementation by running the average fault testing and the break-even treshold. \\
...
Having the two equations (\ref{eq:save_total_cost}, \ref{eq:no_save_total_cost}). 
We can compute

\TODO{Table saving cost}
\TODO{Table loading cost}

\begin{figure}[H]
        \centering
        \includegraphics[width=0.8\linewidth]{img/plots/break_even_focus_run_100000_k_1.png}
        \caption{Treshold for 1 error over a 100.000 runs enclave}
        \label{fig:treshold-k-1}
    \end{figure}
\subsubsection{Timing overhead}
\subsubsection{Data cost}
\subsubsection{Timing cost}
