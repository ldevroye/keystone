...
In addition, the cyber security assumptions are being tested individualy. 
...
%We will make a \textit{static analysis} over the expected outcomes and a \textit{runtime analysis} to check the computation correctness.
%Each test specifies:
%\begin{itemize}
    %\item the enclave workload and macros used setup
    %\item the fault occurence rate
    %\item the metrics recorded (latency, CPU cycles, memory overhead, final success rate when useful, etc.)
%\end{itemize}
