...
RISC-V has three (3) priviliege levels (2.1.2. RISC-V background \cite{keystone-docs, review-keystone}):
\begin{itemize}
...
\begin{itemize}
    \item (2.2.3.1. Creation): The (untrusted) host calls the SM with the Enclave Private Memory (EPM) allocated and the enclave's Page Table (PT).
...
It has three (3) different operating modes:
\begin{itemize}
...
This is done using three (3) components:
\begin{itemize}
...
This chapter describes the concrete implementation, the choices and the assumptions that have been made as well as the questions that are coming along.
...
This isn't the case here as there is only one binary source (the compile code) and it doesn't implement any fault assessment, the enclave behaves as if the fault caused a fatal crash and stops immediatly, returning an error.
...
\subsubsection{Logs}
The compilation logs can be found in the \\
\verb|build-$PLATFORM$BITS/build.log| file.
