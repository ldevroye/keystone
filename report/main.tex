...
\begin{itemize}
    ...
    \item \verb|OCALL_PRINT_BUFFER = 1|: Links the OCALL $1$ to the print wrapper.
...
The edge wrappers are working in parallel to the main loop.
...
\begin{itemize}
    ...
    \item \verb|OCALL_PRINT_BUFFER = 1|
    \item \verb|OCALL_LOAD_CHECKPOINT_BLOB = 8|
    \item \verb|OCALL_SAVE_CHECKPOINT_BLOB = 9|
\end{itemize}
...
\begin{verbatim}
    ...
    2.A check for an existing checkpoint save
    // OCALL the host
    if (load_checkpoint(&checkpoint) == 0) 
    {
        2.B try and restore the saved checkpoint if there is one 
...
\end{verbatim}
Note that we treat a fault as an error as the goal is to checkpoint and not to catch, SEUs, bursts, permanently damaged hardware.
