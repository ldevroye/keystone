...
The files that are implemented and described can be found on the \github and in the \ref{chap:appendix}{Appendix}


\subsection{Rewind \& Checkpointing}
...
The discard and rewind is done by destroying the enclave and building a new one on the the same binary

\subsection{Data encryption}

\subsection{Fault Modeling}
The fault detection and correction is not in the scope of this thesis.
Thus, a small fault model is implemented \ref{file:fault}. it is based of of the the SplitMix \cite{splitmix,splitmix1} algorithm. 
...
We use the 64bit version (SplitMix64).
\\

We maintain a 64-bit \verb|fault_model| structure with three fields:
\begin{itemize}
...
\begin{itemize}
    \item \verb|SEED|: the seed to be picked
    \item \verb|FAULT_RANDOMIZE_SEED|: if set to true then the seed is "randomly" picked, otherwise it is set to \verb|SEED|
