...
\begin{itemize}
    \item \verb|make-examples.sh|: A rather simple one, it serves as the compilation (of the examples \ref{sub:compilation}).
    \item \verb|run-rewind.exp|: The the most complex one as qemu \ref{sub:qemu} is an emulation, inputs (and waiting times) are expected from the user.
    A \verb|.exp| script is perfectly suited, even more since they are the same each time.
    \item \verb|make-run.sh|: This one is a simple concatenation of the other two.
\end{itemize}
All of these can be found in the \github and in the Appendix \ref{file:scripts}.
Also note that the compilation logs produced can be found in the \verb|build-$PLATFORM$BITS/build.log| file.
