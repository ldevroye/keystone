...
The RISC-V architecture, trough Keystone, becomes the testing ground, its physical memory protection and security monitor treated as the only fixed point in a hostile world.
...
Corrupted blobs, truncated writes, and single flipped bits are tested against the sealed with the goal to make sure that the assumptions made are fulfilled with our implementation.
...
The trade-off clear: liveness bought at the price of sealing, unsealing, and a Security Monitor willing to rebuild trust from nothing.
\end{abstract}
...
Therefore, the research should focus on \textit{Fault recovery by rewinding enclaves in a TEE on the RISC-V architecture using Keystone} and extending existing works.
...
PMP as a variable number of entries depending on the platform it is on. 
