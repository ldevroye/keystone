...
\begin{itemize}
  ...
  \item Edge call manipulation such as enclave stall and destruction not in the scope for the same reason.
\end{itemize}
...
The Rewind enclave is evaluated on its overhead computation cycles and data cost.
...
The values below are averaged over $1000$ runs.
They exclude the cached material derivation from the per-part seal and unseal breakdown.
...
\begin{table}[H]
    \centering
    \caption{Other main operation costs}
...
The claims can be checked using the corresponding macros in (Section \ref{sec:macros}).
...
The checkpoint is, at any time, opaque from the host. 
...
Based on the Confidentiality CBC-MAC and CTR scheme, tampering is handled.
\\

Now we have to consider the possible side-attacks, these are that the attackers tries to tamper with the blob directly, which is not plausible under the scheme \textit{over a fixed length message}. 
Another attack is to obtain side informations to deduce information. This can be time to compute the tag or reloading the enclave by destroying it at every save edge call.
The first one is addressed using a simple, home made compare function that always passes over the whole sequence instead of a memcmp() that stops as soon a one byte is wrong.
using memcmp(), or any non constant time comparison function would lead to a time of authentification leak.
...
The atomicity states that we cannot have an undefind saved state. Based on the two last objectives, a maliciously modified state is not something to scared of.
...
great, better (less computation) crypto scheme, how many runs, still useful in practice in case of a full stop of several days for examples
\TODO{time regarding}
\TODO{threat is contained but host is unreliable}
