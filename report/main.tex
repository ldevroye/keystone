...
There are two parts: the \textit{Analysis} (Section \ref{sub:test-analysis}),
where we quantify execution costs and overhead, and the \textit{Cyber security}
(Section \ref{sub:test-cyber}), where we validate the threat model assumptions
and security objectives from Section \ref{sub:threat-model}.
...
\begin{itemize}
    ...
    \item the fault scenario (e.g., forced enclave stop, host crash, power cut)
...
\begin{itemize}
    \item attacker goal (e.g., replaying or corrupting sealed blobs)
    \item the specific security objective under evaluation (confidentiality,
          integrity, atomicity, unclonability)
    \item the expected behavior of the enclave (e.g., detection and rejection of
