...
RISC-V has three (3) priviliege levels (2.1.2. RISC-V background \cite{keystone-web, review-keystone}):
\begin{itemize}
...
The module is not a submodule of Keystone, this allows modularity and force manual updates \cite{keystone-web}. 

\subsection{FireSim}
"FireSim is an FPGA-based cycle-accurate simulator for RISC-V processors. 
Using FireSim, you can test Keystone on open-source processors" (1.2.2 \cite{keystone-web}).
...
In Keystone, it is used to communicate between the host (unsafe) and the encalve (safe). 
...
\cite{keystone-web, review-keystone}.
...
Therefore, a combination including any differing component will give a different key \cite{keystone-web, review-keystone}.
\\
...
More informations about this can be found in the documentation at \textit{5. Attestation} \cite{keystone-web}.
...
\begin{figure}[H]
    ...
    \caption{Key-Hierarchy (6.1. Keystone Key-Hierarchy \cite{keystone-web})}
...
The result of the computation is used as a scalar for the Elliptic-Curve (ECC) Cryptography Key Generator.
...
\begin{figure}[H]
    ...
    \caption{Key Derivation Diagram (6.2 Sealing-Key Derivation \cite{keystone-web})}
...
\section{Compilation}
As shown in the documentation, compilation in Keystone is, at first sight, a simple task (1.2.1.3. Compile Sources \cite{keystone-web}).
